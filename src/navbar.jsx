    ...
    <ContextBox>
      <div className={"flex w-full items-center justify-end gap-8 pr-8 text-[var(--bright-text)]"}>
        {navTabs.map(({ name }, index) => (
          <p key={index} onClick={() => setActiveTabIndex(index)} className={`cursor-pointer heading-font font-semibold ${selectedIndex === index ? "text-[var(--secondary-text)] border-b-2 border-[var(--secondary-text)]" : "text-[var(--bright-text)]"}`}>
            {name}
          </p>
        ))}
      </div>
